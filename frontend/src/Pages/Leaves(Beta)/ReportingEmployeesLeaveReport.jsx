...
export const REPORTING_EMPLOYEE_LEAVE_REPORT='REPORTING_EMPLOYEE_LEAVE_REPORT'
...
export default function ReportingEmployeesLeaveReport() {
...
let year=dayjs().format("YYYY")
  ...
  function fetchleavesData(){
    setIsLoading(true)
    LeavesBetaServices.getReportingEmployeesLeaveReport(year).then((res)=>{
      console.log(res);
        if(res.status===200){
          setIsLoading(false)
...
fetchleavesData()
  ...
  return hasAuthority(REPORTING_EMPLOYEE_LEAVE_REPORT)?( isLoading?<Loading></Loading>:
    
    <Box sx={SerchingComponetsstyle.firstBox}>

         <h4 variant='h6' style={SerchingComponetsstyle.typographystyle}>EMPLOYEES LEAVE REPORT</h4>
